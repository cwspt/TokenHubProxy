...
""";
        ...
        PresetTextBlock.Text = zh
            ? "当前启动器内置了 TokenHub / GLM-5.1、DeepSeek 官方预设，以及 Vinno DeepSeek Relay 下的 deepseek-v4-pro、glm-5-1、minimax-m-2-7 预设。切换预设会自动填充 Base URL、模型名和超时时间，但你仍然可以手工修改。DeepSeek 官方预设使用完整 endpoint：https://api.deepseek.com/chat/completions；Vinno 中转预设使用：https://t.vinno.com/v1/chat/completions。具体模型是否可用取决于当前开放状态和你的账号权限。"
            : "This launcher includes TokenHub / GLM-5.1, official DeepSeek presets, and Vinno DeepSeek Relay presets for deepseek-v4-pro, glm-5-1, and minimax-m-2-7. Switching a preset auto-fills Base URL, model name, and timeout, but you can still edit them manually. Official DeepSeek presets use the full endpoint https://api.deepseek.com/chat/completions; Vinno relay presets use https://t.vinno.com/v1/chat/completions. Actual model availability depends on the current offering and your account permissions.";
        BaseUrlShortcutHeadingTextBlock.Text = zh ? "Base URL 快捷按钮" : "Base URL shortcuts";
        BaseUrlShortcutTextBlock.Text = zh
            ? "Base URL 输入框右侧的 TokenHub / DeepSeek 按钮只替换地址，不会改模型名。它们适合临时切换上游地址；如果你想连模型和超时一起改，请用“上游预设”下拉框。如果手工填入 https://api.deepseek.com，启动器在启动代理或探测时会自动补成 /chat/completions。"
            : "The TokenHub / DeepSeek buttons to the right of Base URL only replace the URL and do not change the model. Use them for quick upstream URL switching; if you want model and timeout changed together, use the Upstream preset dropdown. If you manually enter https://api.deepseek.com, the launcher normalizes it to /chat/completions when probing or starting the proxy.";
