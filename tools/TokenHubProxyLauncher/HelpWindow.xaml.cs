...
""";
        ...
        PresetTextBlock.Text = zh
            ? "当前启动器内置了 TokenHub / GLM-5.1，以及 DeepSeek 的 deepseek-chat、deepseek-reasoner、deepseek-v3、deepseek-v4-flash、deepseek-v4-pro 预设。切换预设会自动填充 Base URL、模型名和超时时间，但你仍然可以手工修改。DeepSeek 预设使用完整 endpoint：https://api.deepseek.com/chat/completions；具体模型是否可用取决于 DeepSeek 当前开放状态和你的账号权限。"
            : "This launcher includes TokenHub / GLM-5.1 plus DeepSeek presets for deepseek-chat, deepseek-reasoner, deepseek-v3, deepseek-v4-flash, and deepseek-v4-pro. Switching a preset auto-fills Base URL, model name, and timeout, but you can still edit them manually. DeepSeek presets use the full endpoint https://api.deepseek.com/chat/completions; actual model availability depends on DeepSeek's current offering and your account permissions.";
